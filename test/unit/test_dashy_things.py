...
def test_valid_odin_address():
    from odinlib import is_valid_odin_address

    main = valid_odin_address()
    test = valid_odin_address('testnet')

    assert is_valid_odin_address(main) is True
    assert is_valid_odin_address(main, 'mainnet') is True
    assert is_valid_odin_address(main, 'testnet') is False

    assert is_valid_odin_address(test) is False
    assert is_valid_odin_address(test, 'mainnet') is False
    assert is_valid_odin_address(test, 'testnet') is True


def test_invalid_odin_address():
    from odinlib import is_valid_odin_address

    main = invalid_odin_address()
    test = invalid_odin_address('testnet')

    assert is_valid_odin_address(main) is False
    assert is_valid_odin_address(main, 'mainnet') is False
    assert is_valid_odin_address(main, 'testnet') is False

    assert is_valid_odin_address(test) is False
    assert is_valid_odin_address(test, 'mainnet') is False
    assert is_valid_odin_address(test, 'testnet') is False
...
def test_parse_masternode_status_vin():
    from odinlib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % odinlib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(odinlib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(odinlib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(odinlib.blocks_to_seconds(16616)) == 2612035
