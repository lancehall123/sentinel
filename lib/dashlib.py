...
def is_valid_odin_address(address, network='mainnet'):
    ...
    odin_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid odin address
        return False

    if (address_version != odin_version):
        return False
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('odin.conf')
