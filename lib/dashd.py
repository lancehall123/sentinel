...
class odindaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some odind methods
    ...
    @classmethod
    def from_odin_conf(self, odin_dot_conf):
        from odin_config import odinConfig
        config_text = odinConfig.slurp_config_file(odin_dot_conf)
        creds = odinConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = odinlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = odinlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = odinlib.blocks_to_seconds(diff)
