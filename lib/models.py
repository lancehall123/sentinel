...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, odind):
        golist = odind.rpc_command('gobject', 'list')
        ...
        for item in golist.values():
            try:
                (go, subobj) = self.import_gobject_from_odind(odind, item)
            except Exception as e:
    ...
    @classmethod
    def import_gobject_from_odind(self, odind, rec):
        import decimal
        import odinlib
        import binascii
        import gobject_json
        ...
        # exclude any invalid model data from odind...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from odind, with every run
        ...
        # get/create, then sync payment amounts, etc. from odind - odind is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(odind)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from odind! %s" % e)
            govobj.vote_delete(odind)
            return (govobj, None)
    ...
    def vote_delete(self, odind):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(odind, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, odind, signal, outcome):
        import odinlib
        ...
        output = odind.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = odinlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(odind, signal)

    def sync_network_vote(self, odind, signal):
        printdbg('\tSyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = odind.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = odind_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 odin addr, non-multisig
            if not odinlib.is_valid_odin_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid odin address for network [%s], returning False" % (self.payment_address, config.network))
                return False
            ...
            # odin Core restricts proposals to 512 bytes max
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (odinlib.blocks_to_seconds(superblockcycle) / 2) +
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = odind_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not odinlib.is_valid_odin_address(addr, config.network):
    ...
    def hash(self):
        import odinlib
        return odinlib.hashit(self.serialise())
