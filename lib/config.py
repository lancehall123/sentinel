...
sentinel_cfg = odinConfig.tokenize(sentinel_config_file)
sentinel_version = "1.4.0"


def get_odin_conf():
    if sys.platform == 'win32':
        odin_conf = os.path.join(os.getenv('APPDATA'), "odinCore/odin.conf")
    else:
        home = os.environ.get('HOME')

        odin_conf = os.path.join(home, ".odincore/odin.conf")
        if sys.platform == 'darwin':
            odin_conf = os.path.join(home, "Library/Application Support/odinCore/odin.conf")

    odin_conf = sentinel_cfg.get('odin_conf', odin_conf)
